...
class QualityFilter:
    ...
    def _format_file(self):
        ...
        for line in self.source_file[1:]:
            lane = line[l_ind]
            samp = line[s_ind]
            if lane not in list(self.QF_from_file.keys()):
                self.QF_from_file[lane] = {}
            self.QF_from_file[lane][samp] = {
                "% Bases >=Q30": line[q_ind],
                "# Reads": line[y_ind],
            }
    ...
    def _logging(self):
        if self.abstract_ext:
            ...
            sys.exit(
                "Yield and Q30 not found in QF file for any of the avalible lanes."
            )
