...
def write_note_to_couch(pid, timestamp, note, lims):
    ...
    url_string = "https://{}:{}@{}".format(
        ...
        config["statusdb"].get("password"),
        config["statusdb"].get("url"),
    )
    couch = couchdb.Server(url=url_string)
    if not couch:
        email_responsible(
            "Connection failed from {} to {}".format(
                lims, config["statusdb"].get("url")
            ),
            "genomics-bioinfo@scilifelab.se",
        )
        email_responsible(
    ...
    if not run_notes_db.get(note["_id"]):
        ...
        if not run_notes_db.get(note["_id"]):
            ...
            time_in_format = datetime.datetime.strftime(
