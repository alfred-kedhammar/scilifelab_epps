...
def main(lims, args):
        ...
        for pool in pools:
            ...
            try:
                ...
                df_pool["transfer_vol_ul"] = minimum(target_amt_fmol / df_pool.conc_nM, df_pool.vol_ul)
            ...
            # If amount is omitted but not target volume, use target volume for calculations
                ...
                df_pool["transfer_vol_ul"] = minimum(pool_target_vol * df_pool.prop_nM_inv, df_pool.vol_ul)
                pool_transfer_vol = sum(df_pool.transfer_vol_ul)
                df_pool["transfer_amt_fmol"] = df_pool.transfer_vol_ul * df_pool.conc_nM
                pool_transfer_amt = sum(df_pool.transfer_amt_fmol)                

            # Evaluate target fraction
            df_pool["molar_percentage"] = df_pool.transfer_amt_fmol / sum(df_pool.transfer_amt_fmol) * 100
            ...
            # Create table
            df_to_print = df_pool.loc[:,["sample_name", "vol_ul", "conc_ng_ul", "size_bp", "conc_nM", "transfer_vol_ul", "transfer_amt_fmol", "molar_percentage"]]
